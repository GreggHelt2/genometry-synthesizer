...
## Phase 6: Advanced & Generative Features (Current)
- [ ] **Generative Matches**: Implement `Generator.js` to find:
    - Primes (`n`, `d` are prime)
    - Twin Primes (`n`, `d` are twin primes)
    - Fibonacci / Golden Ratio relationships
    - Perfect Closure constraints
- [ ] **Generator UI**: Add "Generate" button/modal to Rose Panels to auto-fill parameters based on selected criteria.
- [ ] **Styles**: Implement `StyleManager` to save/load presets (Neon, Pastel, Mono).

## Verification Plan
- **Unit Tests**: Verify `Generator.js` outputs correct primes/twin primes.
- **Manual**: Use the new "Generate" UI to verify it produces valid, closed curves.
